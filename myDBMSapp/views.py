...
def welcome(request):
    return HttpResponse('''<h1>Hello Django Home</h1> <a href="/">home</a> <br> <a href="/courses">courses</a> <br> <a href="/login">login</a>''')
...
def login(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(request, "Welcome You've Successfully Logged-in tho FCIT Help Desk :)")
            return redirect('/started')
        else:
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if joinas == "none":
            messages.success(
                request, "Kindly select why you wanna join FCIT Help Desk ")
            return render(request, 'register.html')
...
def deleteUser(request, pk):
    ...
    if ask == 'Y':
        ...
        messages.success(request, f"User with ID {mail} has removed from the FHD's Database.")
        logout(request)
...
def socialmedia(request, pk):
    ...
    if request.user.is_authenticated:
        user = Register.objects.get(id=pk)
    params = {"user":user,}
    return render(request, 'socialmedia.html', params)